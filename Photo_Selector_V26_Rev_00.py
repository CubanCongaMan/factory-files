#!/usr/bin/env python3
# 03/30/2026 Photo Selector Version 26 Rev 00
...
class App:
    def __init__(self, root):
        ...
        version_label = tk.Label(header, text="v26", font=('Arial', 10), 
...
# chunk 2
    ...
    def build_report_preview_tab(self):
        ...
        self.report_canvas = tk.Canvas(preview_frame, bg='#f5f5f5', width=350, height=450, highlightthickness=2,
                                        highlightbackground=self.accent_color)
        self.report_canvas.pack(fill=tk.BOTH, expand=True, pady=10)
    ...
    def detect_template(self, images):
        ...
        Template 1: 2 photos - FRONT-WV + BACK-WV (same Run)
        Template 2: 3 photos - 1x WV + 2x CU, all FRONT or all BACK (same Run)
        Template 3: 2 photos - 1x WV + 1x CU (same Run)
        Template 4: 4 photos - 4x CU, all FRONT or all BACK (same Run)
        """
        if not images:
            return 1
    ...
    def group_images_by_run(self, image_list):
        """Group images by their RUN number."""
        groups = {}
...
# Chunk 3
    ...
    def update_page_display(self):
        ...
        if current_template == 1:
            ...
            self.report_canvas.create_rectangle(60, 35, 640, 55, fill='#4CAF50', outline='')
            self.report_canvas.create_text(350, 45, text="WIDE-VIEW (Row 2)", font=('Arial', 10, 'bold'), fill='white')
            self.report_canvas.create_rectangle(60, 58, 640, 215, outline="#cccccc", dash=(3,3))
            wv_imgs = page.get('wv_images', [])
            if wv_imgs:
                self.draw_preview_image(wv_imgs[0], 350, 135, max_w=560, max_h=145)
                self.report_canvas.create_text(350, 205, text=os.path.basename(wv_imgs[0]), font=('Arial', 8, 'bold'), fill='#000000')
            else:
            ...
            self.report_canvas.create_text(350, 235, text="CLOSE-UP (Row 4)", font=('Arial', 10, 'bold'), fill='white')
            self.report_canvas.create_rectangle(60, 248, 640, 420, outline="#cccccc", dash=(3,3))
            ...
            wv_imgs = page.get('wv_images', [])
            if wv_imgs:
                self.draw_preview_image(wv_imgs[0], 350, 125, max_w=560, max_h=130)
                self.report_canvas.create_text(350, 190, text=os.path.basename(wv_imgs[0]), font=('Arial', 8, 'bold'), fill='#000000')
            else:
            ...
            self.report_canvas.create_text(350, 220, text="CLOSE-UPS (Row 4 - 2 cells)", font=('Arial', 10, 'bold'), fill='white')
            ...
            self.report_canvas.create_text(350, 45, text="CLOSE-UPS (Row 2 - 2 cells)", font=('Arial', 10, 'bold'), fill='white')
            ...
            self.report_canvas.create_text(350, 230, text="CLOSE-UPS (Row 4 - 2 cells)", font=('Arial', 10, 'bold'), fill='white')
...
# Chunk 5
    ...
    def save_selected(self):
        ...
        for fp in sel:
            ...
            saved += 1
        messagebox.showinfo("Save Complete", f"Saved {saved} files to:\n{self.output_folder}")
    ...
    def rename_one(self, ph, num, total):
        ...
        tk.Label(of, text="Side:", font=('Arial', 10, 'bold'),
                bg=self.bg_color, fg=self.fg_color).pack(side=tk.LEFT, padx=(15,0))
        sd = tk.StringVar(value="FRONT")
        ttk.Combobox(of, textvariable=sd, width=6, state="readonly", 
                    values=["FRONT","BACK"]).pack(side=tk.LEFT, padx=5)
        tk.Label(of, text="View:", font=('Arial', 10, 'bold'),
                bg=self.bg_color, fg=self.fg_color).pack(side=tk.LEFT, padx=(15,0))
        vw = tk.StringVar(value="WV")
        ttk.Combobox(of, textvariable=vw, width=4, state="readonly", 
                    values=["WV","CU"]).pack(side=tk.LEFT, padx=5)
        nm = tk.StringVar()
        tk.Label(d, textvariable=nm, font=('Arial', 11, 'bold'),
                bg=self.bg_color, fg='#4CAF50').pack(pady=10)
        def upn(*a):
            nm.set(f"{self.vb_id.get()}_RUN-{rn.get()}_{sd.get()}-{vw.get()}{os.path.splitext(ph)[1].lower()}")
        rn.trace_add("write", upn)
        sd.trace_add("write", upn)
        vw.trace_add("write", upn)
        upn()
