...
def normalize_supabase_url(raw_url):
    ...
    if not raw:
        return ''
    m = re.search(r'/project/([a-z0-9]+)/', raw, re.IGNORECASE)
    if m:
...
SUPABASE_URL = normalize_supabase_url(
    ...
    'https://supabase.com/dashboard/project/yyoaqeiinwrfdpfpgswc/editor/17626'
)
SUPABASE_KEY = os.environ.get('SUPABASE_KEY', '').strip()
SUPABASE_TABLE = os.environ.get('SUPABASE_TABLE', 'nada1').strip()
SUPABASE_ID_COLUMN = os.environ.get('SUPABASE_ID_COLUMN', 'vb_id')
...
# Chunk 4
    ...
    def apply_template_values(self, text, values):
        updated = text
...
# Chunk No.5 
    ...
    def show_crop_save_dialog(self):
        if not self.original_image:
            return

        win = tk.Toplevel(self.root)
        win.title("Save Cropped Image")
        ...
        tk.Label(
            win,
            text="Crop applied. What would you like to do?",
            font=('Arial', 10, 'bold'),
            bg=self.bg_color,
            fg=self.fg_color,
        ).pack(pady=(20, 8))
        ...
        if self.current_photo_path:
            tk.Label(
                win,
                text=os.path.basename(self.current_photo_path),
                font=('Arial', 9),
                bg=self.bg_color,
                fg='#aabbcc',
                wraplength=380,
        ...
        def save_same():
            if self.save_cropped_image(self.current_photo_path):
                messagebox.showinfo("Saved", f"Cropped image saved:\n{self.current_photo_path}")
                self.status.config(text=f"Saved cropped image: {os.path.basename(self.current_photo_path)}")
            win.destroy()

        def save_as_new():
            new_path = self.get_crop_save_as_path()
            if new_path and self.save_cropped_image(new_path):
                messagebox.showinfo("Saved", f"Cropped image saved:\n{new_path}")
                self.status.config(text=f"Saved cropped image as: {os.path.basename(new_path)}")
            win.destroy()

        def skip_save():
            self.status.config(text="Crop applied (not saved to disk)")
            win.destroy()

        self.create_styled_button(btn_frame, "Save Same Name", save_same, 14).pack(side=tk.LEFT, padx=6)
        self.create_styled_button(btn_frame, "Save As...", save_as_new, 12).pack(side=tk.LEFT, padx=6)
        self.create_styled_button(btn_frame, "Skip", skip_save, 10).pack(side=tk.LEFT, padx=6)

        win.wait_window()

    def get_crop_save_as_path(self):
        default_path = self.current_photo_path or "cropped_image.jpg"
        base_dir = os.path.dirname(default_path) if default_path else ""
        base_name = os.path.basename(default_path) if default_path else "cropped_image.jpg"
        stem, ext = os.path.splitext(base_name)
        default_name = f"{stem}_cropped{ext or '.jpg'}"
    ...
    def save_cropped_image(self, output_path):
        if not self.original_image or not output_path:
            return False
        try:
            self.original_image.save(output_path)
            return True
        except Exception as e:
            messagebox.showerror("Save Error", f"Could not save cropped image:\n{e}")
            return False
