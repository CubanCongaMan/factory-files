...
class ImageProcessor:
    ...
    def crop_image(self, image_path, output_dir):
        ...
        with Image.open(image_path) as img:
            ...
            crop_width = min(self.cfg.RIGHT_CROP_X, width)
            ...
            right_peaks = self._detect_tick_peaks(
                gray,
                max(0, boundary_x - self.LEFT_STRIP_W),
                min(crop_width, boundary_x + 1),
            )
            ...
            while (start + window_height) <= height and scan_guard < 2000:
                ...
                if ticks >= self.TARGET_TICKS:
                    ...
                    # Keep filename depth format compatible with downstream parsing.
                    start_ft = max(0.0, (start - self.cfg.DEPTH_OFFSET) / self.cfg.PIXELS_PER_FOOT)
                    end_ft = max(start_ft, (end - self.cfg.DEPTH_OFFSET) / self.cfg.PIXELS_PER_FOOT)
                    filename = f"{basename}_{start_ft:.1f}-{end_ft:.1f}ft.jpg"
