...
class Config:
    ...
    DEPTH_OFFSET = -74
    HEADER_HEIGHT = 110
    # First-page title safety pad: include extra top band so the
    # 'Depth and Image-NM' label is not clipped in first-page crop output.
    FIRST_PAGE_HEADER_EXTRA_INCHES = 0.2
    RIGHT_CROP_X = 685
    LABEL_OFFSET = 20
    IMAGE_HEIGHT_INCHES = 7.0
    # Report image sizing profile lock:
    # choose one profile to prevent accidental behavior drift across edits.
    REPORT_IMAGE_PROFILE = "wide_min_width"
    REPORT_IMAGE_PROFILES = {
        "wide_min_width": {
            "max_height_inches": 6.8,
            "min_width_inches": 4.2,
            "dpi": 300,
            "enforce_min_width": True,
        },
        "height_only": {
            "max_height_inches": 6.8,
            "min_width_inches": 0.0,
            "dpi": 300,
            "enforce_min_width": False,
        },
        "legacy_fixed_7x4p2": {
    ...
    @classmethod
    def get_report_image_profile(cls):
        """Return active report image sizing profile with safe fallback."""
        profile = cls.REPORT_IMAGE_PROFILES.get(cls.REPORT_IMAGE_PROFILE)
        if profile is None:
            profile = cls.REPORT_IMAGE_PROFILES["wide_min_width"]
        return dict(profile)

    # Legacy aliases retained for compatibility with older code paths.
    REPORT_IMAGE_MAX_HEIGHT_INCHES = 6.8
    REPORT_IMAGE_MIN_WIDTH_INCHES = 4.2
    REPORT_IMAGE_DPI = 300
...
class ImageProcessor:
    ...
    def _detect_tick_peaks(self, gray_img, x_start, x_end, y_start=0, y_end=None):
        """Return ruler tick peak rows in a grayscale image strip."""
        pix = gray_img.load()
        ...
        for y in range(ys, ye):
            ...
            for x in range(x0, x1):
                if pix[x, y] < self.TICK_DARK_THRESHOLD:
                    dark += 1
            scores.append(dark)
        ...
        for i in range(1, len(scores) - 1):
            if (
                scores[i] >= self.TICK_MIN_DARK_PIXELS
                and scores[i] >= scores[i - 1]
                and scores[i] >= scores[i + 1]
            ):
                y = ys + i
                if not peaks or (y - peaks[-1]) > self.TICK_PEAK_MIN_SEP:
                    peaks.append(y)
        return peaks
    ...
    def _find_ruler_boundary_x(self, gray_img):
        ...
        w, _ = gray_img.size
        left_transition, _ = self._find_ruler_transitions_x(gray_img)
        if left_transition is not None:
            return left_transition
        return w // 2

    def _find_ruler_transitions_x(self, gray_img):
        """Detect left/right ruler transitions from column brightness profile."""
        w, h = gray_img.size
        pix = gray_img.load()
        y0 = max(0, h // 4)
        y1 = min(h, (3 * h) // 4)
        ...
        # Left transition: bright ruler panel -> darker borehole texture.
        ...
        # Right transition: darker borehole texture -> bright right ruler panel.
        right_transition = None
        for x in range(w - 11, 9, -1):
            if means[x] < 200 and means[x + 1] > 220:
                right_transition = x + 1
                break

        return left_transition, right_transition

    def count_left_ticks(self, gray_crop):
        """Count tick marks on the left ruler only."""
        w, h = gray_crop.size
        left = self._detect_tick_peaks(
            gray_crop,
            0,
            min(self.LEFT_STRIP_W, w),
            y_start=0,
            y_end=h,
        )
        return len(left)

    def _estimate_tick_spacing(self, peaks):
        if len(peaks) < 3:
            return None
    ...
    def _find_horizontal_axis_divider_row(self, gray_img):
        """Find the bottom divider line under the horizontal angle scale."""
        w, h = gray_img.size
        y_start = 20
        y_end = min(h, 500)
        if y_end <= y_start:
    ...
    def crop_image(self, image_path, output_dir):
        ...
        with Image.open(image_path) as img:
            ...
            # Ensure deterministic outputs per run.
            ...
            left_transition_x, right_transition_x = self._find_ruler_transitions_x(full_gray)

            # Dynamic horizontal crop lock:
            # include full televiewer span from left 0 through right-side 0 ruler.
            x0 = 0
            if right_transition_x is None:
                x1 = width
            else:
                # Keep right ruler ticks visible by adding a small margin.
                x1 = min(width, right_transition_x + (2 * self.LEFT_STRIP_W))

            # Safety fallback for unexpected layouts.
            if (x1 - x0) < max(200, width // 3):
                x1 = width

            working = img.crop((x0, 0, x1, height))
            crop_width = working.size[0]
            gray = working.convert('L')

            if right_transition_x is not None:
                right_transition_local = max(0, min(crop_width - 1, right_transition_x - x0))
            else:
                right_transition_local = crop_width - 1

            left_peaks = self._detect_tick_peaks(gray, 0, min(self.LEFT_STRIP_W, crop_width))
            right_peaks = self._detect_tick_peaks(
                gray,
                max(0, right_transition_local - self.LEFT_STRIP_W),
                min(crop_width, right_transition_local + self.LEFT_STRIP_W + 1),
            )

            spacing = self._estimate_tick_spacing(left_peaks) or self._estimate_tick_spacing(right_peaks)
            if spacing is None:
                raise ValueError("Could not detect ruler tick spacing from source image.")

            target_ticks_one_side = int(3.0 * self.TICKS_PER_FOOT_PER_SIDE)
            ...
            if left_peaks and right_peaks:
                first_peak = min(left_peaks[0], right_peaks[0])
            elif left_peaks:
                first_peak = left_peaks[0]
            elif right_peaks:
                first_peak = right_peaks[0]
            else:
                first_peak = 0
            ...
            # First-page special crop: preserve full top horizontal axis/title
            # while limiting borehole depth content to 0.5-2.5 ft.
            first_start_ft = 0.5
            first_end_ft = 2.5
            pixels_per_foot_dynamic = spacing * self.TICKS_PER_FOOT_PER_SIDE
            body_start = int(round(start + (first_start_ft * pixels_per_foot_dynamic)))
            body_end = int(round(start + (first_end_ft * pixels_per_foot_dynamic)))
            body_start = max(0, min(height - 1, body_start))
            body_end = max(body_start + 1, min(height, body_end))

            pixels_per_inch = self.cfg.PIXELS_PER_FOOT / 12.0
            extra_header_px = int(round(max(0.0, self.cfg.FIRST_PAGE_HEADER_EXTRA_INCHES) * pixels_per_inch))
            base_header_end = self.cfg.HEADER_HEIGHT + extra_header_px
            # Preserve the full horizontal angle scale band (0/90/180/270/0)
            # by retaining content down to the scale divider line.
            axis_divider_row = self._find_horizontal_axis_divider_row(gray)
            axis_band_end = (axis_divider_row + 1) if axis_divider_row is not None else first_peak
            header_end = min(height, max(base_header_end, axis_band_end))

            first_crop_height = header_end + (body_end - body_start)
            first_crop = Image.new('RGB', (crop_width, first_crop_height), 'white')
            first_header = img.crop((0, 0, crop_width, header_end))
            first_body = img.crop((0, body_start, crop_width, body_end))
            first_crop.paste(first_header, (0, 0))
            first_crop.paste(first_body, (0, header_end))

            first_filename = f"{basename}_{first_start_ft:.1f}-{first_end_ft:.1f}ft.jpg"
            first_filepath = os.path.join(output_dir, first_filename)
            first_crop.save(first_filepath, 'JPEG', quality=95)
            output_files.append(first_filepath)

            overlap_feet = max(0.0, float(self.cfg.OVERLAP_FEET))
            overlap_px_dynamic = int(round(overlap_feet * pixels_per_foot_dynamic))

            # Start page-2 cropping with controlled overlap.
            start = max(0, body_end - overlap_px_dynamic)

            # Filename interval labels for non-first pages.
            label_start_ft = max(0.0, first_end_ft - overlap_feet)
            ...
            while (start + window_height) <= height and scan_guard < 2000:
                ...
                ticks = self.count_left_ticks(gray_crop)

                expanded = 0
                while ticks < self.TARGET_TICKS and end < height and expanded < max_expand:
                ...
                if ticks >= self.TARGET_TICKS:
                    ...
                    # Keep labels in strict 3-ft intervals for report workflow.
                    label_end_ft = label_start_ft + self.cfg.FEET_PER_PAGE
                    filename = f"{basename}_{label_start_ft:.1f}-{label_end_ft:.1f}ft.jpg"
                    filepath = os.path.join(output_dir, filename)

                    seg.save(filepath, 'JPEG', quality=95)
                    output_files.append(filepath)
                    next_start = max(0, end - overlap_px_dynamic)
                    if next_start <= start:
                        next_start = start + 1
                    start = next_start
                    label_start_ft = label_end_ft
                else:
                    start += step
...
class ReportGenerator:
    ...
    def __init__(self):
        self.cfg = Config()
        self.image_profile = self.cfg.get_report_image_profile()

    def report_max_height_inches(self):
        """Return configured max insertion height in inches."""
        return float(self.image_profile.get("max_height_inches", self.cfg.REPORT_IMAGE_MAX_HEIGHT_INCHES))

    def report_min_width_inches(self):
        """Return configured minimum visual width in inches."""
        return float(self.image_profile.get("min_width_inches", self.cfg.REPORT_IMAGE_MIN_WIDTH_INCHES))

    def report_dpi(self):
        """Return configured report-ready DPI."""
        return int(self.image_profile.get("dpi", self.cfg.REPORT_IMAGE_DPI))

    def enforce_report_min_width(self):
        """Return whether horizontal widening should be applied."""
        return bool(self.image_profile.get("enforce_min_width", True))
    ...
    def is_auto_clone_template(self, doc):
        """Return True if template supports per-image page cloning."""
        return self.is_canonical_template(doc) or self._is_single_page_wide_slot_variant(doc)
    ...
    def collect_single_wide_image_cells(self, doc):
        """Collect one image cell per page for 1x1 wide-slot variants."""
        cells = []
        for table in doc.tables:
            if len(table.rows) == 1 and len(table.columns) == 1:
                cells.append(table.rows[0].cells[0])
        return cells

    def _is_single_page_wide_slot_variant(self, doc):
        """Return True for 3-table templates with a final 1x1 image slot.

        This supports the user variant where canonical Table 3 is changed from
        1x2 to a single wide 1x1 cell.
        """
        if len(doc.tables) != 3:
            return False
        shapes = [(len(t.rows), len(t.columns)) for t in doc.tables]
        return shapes == [(2, 2), (5, 4), (1, 1)]

    @staticmethod
    def _looks_like_image_placeholder(text):
        """Return True when a 1x1 cell text suggests an image target."""
    ...
    def apply_containment_lock(self, doc):
        """Harden layout on image-slot tables only.

        Keep Tables 1 and 2 fully template-driven by limiting containment
        changes to image tables that receive pictures/notes.
        """
        target_indices = set()

        if self.is_canonical_template(doc):
            target_indices = {
                i for i, table in enumerate(doc.tables)
                if len(table.rows) == 1 and len(table.columns) == 2
            }
        elif self._is_single_page_wide_slot_variant(doc):
            target_indices = {
                i for i, table in enumerate(doc.tables)
                if len(table.rows) == 1 and len(table.columns) == 1
            }
            ...
            target_indices = set(modern_indices + legacy_indices)

        for i, table in enumerate(doc.tables):
            if i not in target_indices:
                continue

            # Keep image-table geometry stable instead of letting Word re-autofit.
            table.autofit = False
    ...
    def find_image_cells(self, doc):
        ...
        modern_indices = []
        legacy_indices = []
        one_by_one_count = sum(
            1 for t in doc.tables if len(t.rows) == 1 and len(t.columns) == 1
        )
        single_page_wide = self._is_single_page_wide_slot_variant(doc)
        for i, table in enumerate(doc.tables):
            ...
            # the final trailing notes table. Also allow user wide-slot variants
            # that intentionally use a final 1x1 image table.
            if len(table.rows) == 1 and len(table.columns) == 1:
                cell_text = (table.rows[0].cells[0].text or "").strip()
                is_last_table = i == (len(doc.tables) - 1)

                if self._looks_like_image_placeholder(cell_text):
                    legacy_indices.append(i)
                elif not cell_text:
                    if not is_last_table:
                        legacy_indices.append(i)
                    elif single_page_wide or one_by_one_count == 1:
                        # Single 1x1 in the template is likely the image slot,
                        # not a dedicated trailing notes container.
                        legacy_indices.append(i)
    ...
    def prepare_report_images(self, images):
        """Create report-ready JPEGs with controlled aspect and DPI.

        The report pipeline uses these temporary files for Word insertion only.
        """
        if not images:
            return []

        src_dir = os.path.dirname(images[0]) or '.'
        ready_dir = os.path.join(src_dir, "_report_ready_300dpi")
        ...
        for name in os.listdir(ready_dir):
            ...
            if lower.endswith('.jpg') or lower.endswith('.jpeg'):
                try:
                    os.remove(os.path.join(ready_dir, name))
                except OSError:
                    pass

        prepared = []
        target_dpi = (self.report_dpi(), self.report_dpi())
        max_h = self.report_max_height_inches()
        min_w = self.report_min_width_inches()
        min_aspect = (min_w / max_h) if max_h > 0 else 0.0

        for image_path in images:
            out_path = os.path.join(ready_dir, os.path.basename(image_path))
            with Image.open(image_path) as img:
                fixed = img.convert('RGB')
                w, h = fixed.size
                if w > 0 and h > 0:
                    aspect = w / h
                    if self.enforce_report_min_width() and min_aspect > 0 and aspect < min_aspect:
                        target_w = max(w, int(round(h * min_aspect)))
                        # Restore wider image presentation while preserving
                        # insertion height lock and 300-DPI export quality.
                        fixed = fixed.resize((target_w, h), Image.Resampling.LANCZOS)
                fixed.save(out_path, 'JPEG', quality=95, dpi=target_dpi)
    ...
    def find_notes_table(self, doc):
        ...
        empty_one_by_one = [
            ...
            and not table.rows[0].cells[0].text.strip()
        ]
        if len(empty_one_by_one) == 1 and empty_one_by_one[0] == len(doc.tables) - 1:
            if self._is_single_page_wide_slot_variant(doc):
                return None
    ...
    def generate(self, template_path, images, header_data, output_path):
        ...
        single_wide_mode = self._is_single_page_wide_slot_variant(doc)
        report_images = self.prepare_report_images(images)

        if canonical_mode or single_wide_mode:
            if not images:
                raise ValueError("No cropped images generated for report.")

            # Build N pages from one single-page template.
            ...
            if canonical_mode:
                ...
                image_cells = self.collect_single_wide_image_cells(doc)
            ...
            for i, (cell, img_path) in enumerate(zip(image_cells, report_images)):
                step += 1
                ...
                target_height = self.report_max_height_inches()
                if same_page_notes and i == (len(report_images) - 1):
                    target_height = min(target_height, last_image_height_inches)
            ...
            if same_page_notes:
                ...
                if canonical_mode:
                    ...
                    note_cell = self.collect_single_wide_image_cells(doc)[-1]
        ...
        for i, (idx, img_path) in enumerate(zip(image_indices[:len(images)], report_images)):
            step += 1
            Console.progress(step, total_steps, "Building report")
...
""")
    ...
    def generate_report(self, header_data):
        ...
        try:
            ...
            if generator.is_auto_clone_template(template_doc):
