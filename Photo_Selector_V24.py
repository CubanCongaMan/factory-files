#03/28/2026 Photo Selector Version 24
#Author: RLS & Factory AI Droids
#Fix: Report Preview filenames now black bold for better visibility
...
class App:
    def __init__(self, root):
        ...
        version_label = tk.Label(header, text="v24", font=('Arial', 10), 
    ...
    def build_photo_selector_tab(self):
        ...
        main = tk.Frame(self.tab1, bg=self.bg_color)
        ...
        right = tk.Frame(main, width=400, bg=self.bg_color)
        ...
        self.pcv = tk.Canvas(pf, bg='#243554', width=360, height=260, highlightthickness=0)
    ...
    def build_report_preview_tab(self):
        main = tk.Frame(self.tab2, bg=self.bg_color)
        ...
        self.report_canvas = tk.Canvas(preview_frame, bg='#f5f5f5', width=700, height=450, highlightthickness=2,
                                        highlightbackground=self.accent_color)
    ...
    def upd_pv(self):
        ...
        im.thumbnail((int(340*self.zoom), int(340*self.zoom)))
        self.preview_img = ImageTk.PhotoImage(im)
        self.pcv.delete("all")
        self.pcv.create_image(180+self.pan_x, 130+self.pan_y, image=self.preview_img)
    ...
    def apply_crop(self):
        ...
        md = int(340*self.zoom)
        sc = min(md/iw, md/ih)
        dw, dh = int(iw*sc), int(ih*sc)
        il, it = 180+self.pan_x-dw//2, 130+self.pan_y-dh//2
        x1,y1,x2,y2 = self.crop_coords
